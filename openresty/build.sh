...
DATE_TAG=$(date +%Y%m%d)
OPENRESTY_VERSION=$(grep -m1 'OPENRESTY_VERSION=' Dockerfile | sed 's/.*=//' | sed 's/[[:space:]]*\\\s*$//' | sed 's/^[[:space:]]*//')
PLATFORM="linux/amd64,linux/arm64"

DEFAULT_TAG="${REGISTRY}/${IMAGE_NAME}:${OPENRESTY_VERSION}-${DATE_TAG}"
TAG="${DEFAULT_TAG}"
...
echo "============================================"
echo " Version:  ${OPENRESTY_VERSION}"
echo " Image:    ${TAG}"
